...
class ApiController extends SessionsDatabase {
    ...
    async sendMessage(req, res) {
        ...
        console.log("🚀 ~ ApiController ~ sendMessage ~ asolo:", asolo)
        if (!asolo.status) return res.status(400).json(asolo);
        try {
            let client = new Client(asolo.session, receiver);
            console.log("🚀 ~ ApiController ~ sendMessage ~ client:", client)
    ...
    async sendMedia(req, res) {
        ...
        try {
            ...
            if (waiting) {
                ...
                return res
                    .status(200)
                    .json({ status: true, message: "Media is being sent." });
            } else {
                await client
                    .sendMedia(data.url, data.caption, opts)
                    .then(() => {
                        return res
                            .status(200)
                            .json({ status: true, message: "Media sent." });
                    })
                    .catch((e) => {
                        return res.status(400).json({
                            status: false,
            ...
            return res
                .status(400)
                .json({ status: false, message: "Failed to send media." });
        }
    }
        ...
        try {
            ...
            await client
                ...
                .then(() => {
                    return res
                        .status(200)
                        .json({ status: true, message: "Button sent." });
                })
                .catch((e) => {
                    return res.status(400).json({
                        status: false,
                        message: "Failed to send button.",
                        error: e,
                    });
                });
        } catch (e) {
        ...
        try {
            ...
            await client
                ...
                .then(() => {
                    return res
                        .status(200)
                        .json({ status: true, message: "List Message sent." });
                })
                .catch((e) => {
                    return res.status(400).json({
                        status: false,
    ...
    async fetchGroup(req, res) {
        ...
        try {
            ...
            return res
                .status(400)
                .json({ status: false, message: "Failed to fetch group." });
        }
    }
    ...
    async trigerCampaigns(req, res) {
        ...
        if (!asolo.status) return res.status(400).json(asolo);
        try {
            const eventEmitter = require("../lib/Event.js");
    ...
    async actPlugin(req, res) {
        ...
        try {
            ...
            let command = commands.get(
                commands_name.toLowerCase().replace(/\s/g, "")
            );
            if (!command)
                return res
                    .status(400)
                    .json({ status: false, message: "Invalid command." });

            if (status == "inactive") {
                await actSessionCommands(
                    dbsession.id,
                    "delete",
                    commands_name.toLowerCase().replace(/\s/g, "")
                )
                    .then((data) => {
                        if (!data)
                            return res.status(400).json({
                                status: false,
                                message:
                                    "Failed to activate plugin, because device is not connected.",
                            });
                        return res.status(200).json({
                            status: true,
                        });
    ...
    async asolo(req, res, validator = true) {
        ...
        try {
            ...
            if (dbsession.status !== "CONNECTED")
                return { status: false, message: "Device is stopped." };

            let session = await new SessionConnection(socket).getSession(
                dbsession.id
            );
            if (!session)
                return { status: false, message: "Session is stopped." };
